package com.zhishangquan.server.service.pay;
...
import com.zhishangquan.server.request.pay.WxPayOrderQueryRequestBean;
import com.zhishangquan.server.request.pay.WxPayOrderRefundRequestBean;
import com.zhishangquan.server.request.pay.WxPayUnifiedOrderRequestBean;
import com.zhishangquan.server.response.pay.WxPayOrderQueryResponseBean;
import com.zhishangquan.server.response.pay.WxPayOrderRefundResponseBean;
import com.zhishangquan.server.response.pay.WxPayUnifiedOrderResponseBean;
...
public class WxPayService {
    ...
    //回调路径
    //private static final String NOTIFY_URL = "http://yeoner-appserver-dev.obaymax.com/mall/wxPayNotify";
     ...
     * 拿到prepayid后需要再一次加密 生成带签名的客户端支付信息
    ...
    public Map<String, Object> genPrepayMap(PayParamBean param) throws Exception {
        ...
        map2.put("noncestr", StringUtil.getUUID(32));
    ...
    public String genUnifiedorder(PayParamBean paramBean) throws Exception {
        ...
        unifiedOrderBean.setNonce_str(StringUtil.getUUID(32));
        unifiedOrderBean.setOut_trade_no(paramBean.getOrder_no());
        unifiedOrderBean.setBody("WX_ORDER");  //业务中不需要，可随意
        ...
        //获取签名
        String sign = Signature.getSign(unifiedOrderBean, "key=" + weixinConfig.getApi_key()).toUpperCase();
        logger.info("genUnifiedorder 生成预支付订单的签名：" + sign);
        unifiedOrderBean.setSign(sign);

        //转换成xml格式
        String param = XmlUtil.toXml(unifiedOrderBean);
        ...
        WxPayUnifiedOrderResponseBean bean = XmlUtil.toObject(responseXml, WxPayUnifiedOrderResponseBean.class);
        String prepayId = "";
     ...
     * @throws Exception
     */
    public PayStatusBean wxPayQuery(String orderNo) throws Exception {
        ...
        queryRequstBean.setNonce_str(StringUtil.getUUID(32));
        queryRequstBean.setSign(Signature.getSign(queryRequstBean, "key=" + weixinConfig.getApi_key()).toUpperCase());

        String param = XmlUtil.toXml(queryRequstBean);
        logger.info("微信支付查询订单参数：" + param);

        //返回请求结果
        String responseXml = postXml(weixinConfig.getQuery_url(), param.toString());
        //将返回结果转为对象
        WxPayOrderQueryResponseBean queryResponseBean = XmlUtil.toObject(responseXml,
                WxPayOrderQueryResponseBean.class);
        ...
        if (queryResponseBean.getReturn_code().equals("SUCCESS")) {
            if (queryResponseBean.getResult_code().equals("SUCCESS")) {
                ...
                logger.info("取微信支付查询订单失，订单号：" + orderNo + "; 错误码:" + queryResponseBean.getErr_code() + "; 错误描述"
                        + queryResponseBean.getErr_code_des());
            }
        } else {
        ...
        if (!trade_state.equals("SUCCESS") && !trade_state.equals("USERPAING") && !trade_state.equals("NOTPAY")) {
            trade_state = "FAIL";
        }

        PayStatusBean bean = new PayStatusBean();
    ...
    public String wxPayRefund(String orderNo, Integer amount) throws Exception {
        ...
        refundRequstBean.setNonce_str(StringUtil.getUUID(32));
        //生成一个随机的退款单号
        String out_refund_no = StringUtil.getUUID(32);
        ...
        refundRequstBean.setSign(Signature.getSign(refundRequstBean, "key=" + weixinConfig.getApi_key()).toUpperCase());

        String param = XmlUtil.toXml(refundRequstBean);
        ...
        //将返回结果转为对象
        WxPayOrderRefundResponseBean refundResponseBean = XmlUtil.toObject(responseXml,
                WxPayOrderRefundResponseBean.class);
        if (refundResponseBean.getReturn_code().equals("SUCCESS")) {
            if (refundResponseBean.getResult_code().equals("SUCCESS")) {
                ...
                logger.info("微信退款申请失败，订单号：" + orderNo + "; 错误码:" + refundResponseBean.getErr_code() + "; 错误描述"
                        + refundResponseBean.getErr_code_des());
            }
        } else {
    ...
    //	 */
    //	private static String checkFilePath() throws URISyntaxException {
    //		String filePath = PropertiesUtil.class.getClassLoader().getResource("").toURI().getPath();
